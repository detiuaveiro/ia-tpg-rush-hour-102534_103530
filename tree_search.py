...
class Matrix:
    def __init__(self, grid, action=[], parent=None, cost=0, heuristic=0, cursor=(2, 3)):
        if " " in grid:
...
class SearchTree:
    ...
    def search3(self):
        while self.open_nodes != []:
            ...
            for a in AI.actions(node):
                ...
                new_cost = node.cost + len(path)
                if newgrid in self.grids_visited:
                    if new_cost >= self.total_costs[newgrid]:
                        continue
                else:
                    self.grids_visited.add(newgrid)
                newnode = Matrix(newgrid, [a], node, new_cost)
                newnode.cursor = (cursorx, cursory)
                newnode.set_bounds(a[0], bounds)
    ...
    def add_to_open(self, lnewnodes):
        if self.strategy == 'breadth':
            ...
            pass # a min heap já está ordenada
        elif self.strategy == 'uniform':
            pass # a min heap já está ordenada

def main():
    if False: # Solução SÓ com breadth
        with open("levels1.txt", "r") as f:
            levels = f.readlines()
            total_time = 0.0
            total_moves = 0
            for level in levels:
